...
function addData() {
    let student_records = new Array()
    student_records.push({
        studentData
    })
    localStorage.setItem("students", JSON.stringify(student_records));
    getData()
}

function getData() {
    let retriveData = localStorage.getItem("students")
    let data = JSON.parse(retriveData);
    let arr = data[0].studentData
    let arrlength = (data[0].studentData).length;
    ...
    for (let i = 0; i < arrlength; i++) {
        ...
        <td>${arr[i].studentAge}</td>
        <td>${arr[i].studentGender}</td>
        <td>${arr[i].studentMobileNumber}</td>
        <td>${arr[i].studentEmail}</td>
        </tr>`
    }
    studentTable += `</table>`
    tableDiv.innerHTML = studentTable
